...
void ColdFilter::update(const uint32_t packet_id, const uint32_t flow_label, uint32_t weight) {
    bool flag = false;
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        for (int j = 0; j < d; ++j) {
            ...
            index_hash = 0;
            MurmurHash3_x86_32(&flow_label, 4, seed, &index_hash);
...
int ColdFilter::report(const uint32_t flow_label) {
    ...
    for (int i = 0; i < 2; ++i) {
        uint32_t min_value = UINT32_MAX;
        for (int j = 0; j < d; ++j) {
            MurmurHash3_x86_32(&flow_label, 4, i * 10 + j, &index_hash);
