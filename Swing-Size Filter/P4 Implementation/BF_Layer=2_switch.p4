...
struct hash_info_t{
	...
	bit<16> counter_index2;
	bit<b1> counter_value1;
	bit<b2> counter_value2;
	bit<1> random_value;
	bit<1> op;
	bit<16> sketch_index;
	bit<1> bingo;
...
control MyIngress(inout headers hdr,
	...
	CRCPolynomial<bit<32>>(coeff=0x401CDD1A,reversed=true, msb=false, extended=false, init=0xFFFFFFFF, xor=0xFFFFFFFF) crc01;
	...
	// hash functions in L2
	CRCPolynomial<bit<32>>(coeff=0x22312215,reversed=true, msb=false, extended=false, init=0xFFFFFFFF, xor=0xFFFFFFFF) crc3;
	CRCPolynomial<bit<32>>(coeff=0x6678D1BA,reversed=true, msb=false, extended=false, init=0xFFFFFFFF, xor=0xFFFFFFFF) crc4;
	...
	Register<bit<b2>, bit<16>>(M2, median2) L2;
	
	Register<bit<32>, bit<32>>(W) sketch;
	
	Hash<bit<1>>(HashAlgorithm_t.CUSTOM, crc0) hash00;
	Hash<bit<1>>(HashAlgorithm_t.CUSTOM, crc01) hash001;
	
	Hash<bit<16>>(HashAlgorithm_t.CUSTOM, crc1) hash01;
	Hash<bit<1>>(HashAlgorithm_t.CUSTOM, crc2) hash02;
	
	Hash<bit<16>>(HashAlgorithm_t.CUSTOM, crc3) hash11;
	Hash<bit<1>>(HashAlgorithm_t.CUSTOM, crc4) hash12;
	...
	RegisterAction<bit<b2>, bit<16>, bit<b2>>(L2) L2_alu_write =  {
		void apply(inout bit<b2> register_data, out bit<b2> result) {
			if (meta.hash_info.op == 0) {
				register_data = register_data |-| 1;
			} else {
				register_data = register_data |+| 1;
	...
	*/
	...
	action hash_L2 () {
		meta.hash_info.random_value = hash001.get({hdr.ipv4.srcAddr, hdr.ipv4.dstAddr});
		meta.hash_info.counter_index2 = hash11.get({hdr.ipv4.srcAddr, meta.hash_info.random_value});
		meta.hash_info.op = hash12.get({hdr.ipv4.srcAddr, meta.hash_info.counter_index2});
	}
	...
	table hash_L2_table {
		...
		size = 1;
		const default_action = hash_L2();
	}
	
	action L2_cal() {
		meta.hash_info.counter_value2 = L2_alu_write.execute(meta.hash_info.counter_index2);
	}
    ...
    apply {
        ...
        if (hdr.ipv4.isValid()){
			...
			if (meta.hash_info.counter_value1 == 0 || meta.hash_info.counter_value1 == T1) {
				hash_L2_table.apply();
				L2_cal_table.apply();
				if (meta.hash_info.counter_value2 == 0 || meta.hash_info.counter_value2 == T2) {
